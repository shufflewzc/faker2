...
grep 'shufflewzc_0109' /ql/data/config/task_before.sh >/dev/null 2>&1 || grep 'shufflewzc_0109' /ql/config/task_before.sh > /dev/null 2>&1
if [[ $? != 0 ]];then
 cp /ql/repo/shufflewzc_faker2_main/docker/task_before.sh /ql/config/ >/dev/null 2>&1 || cp /ql/data/repo/shufflewzc_faker2_main/docker/task_before.sh /ql/data/config/
fi
...
[[ -d `echo /ql/data/log/shufflewzc_faker2_main_jd_sharecode*|awk '{print $1}'` ]]  && dir_code=`ls -dt /ql/data/log/shufflewzc_faker2_main_jd_sharecode*|awk '{print $1}'|head -1`

## 预设的仓库及默认调用仓库设置
## 将"repo=$repo1"改成repo=$repo2"或其他，以默认调用其他仓库脚本日志
## 也可自行搜索本脚本内的"name_js=("和"name_js_only",将"repo"改成"repo2"或其他，用以自由组合调用仓库的脚本日志
repo1='shufflewzc_faker2_main'                       #预设的 faker2 仓库
